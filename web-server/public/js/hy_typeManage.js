...
var typeData = [];
//2.1.根据id删除数组元素
typeData.deleteById = function (id) {
...
typeData.update = function (model) {
    for (var i = 0; i < this.length; i++) {
...
//2-5.返回一页数据
if( !typeData.pageIndex)
{
    typeData.pageIndex = 1;//页码
}

typeData.pageCount = 5;//每页记录条数
typeData.pages=0;//总页数
//返回第pageIndex页数据
typeData.getPageData = function () {
    var pageData = [];
...
function loadDetailData() {
    ...
    var tbList = getElement("typeList");//获得表对象
    for (var i = tbList.rows.length - 1; i >= 0; i--) {//删除表所有行
        tbList.deleteRow(i);
    }
    var arrPage = typeData.getPageData();//获得一页数据
...
function loadDataToTb(arr) {
    ...
    typeData.pages = typeData.length % typeData.pageCount != 0 ? (typeData.length - typeData.length % typeData.pageCount) / typeData.pageCount + 1 : typeData.length / typeData.pageCount;
    getElement('currentPage').innerHTML = typeData.pageIndex;
    getElement('allPage').innerHTML = typeData.pages;
}
//4-1.插入一行,model-数组元素
function addTr(model) {
    var tb = getElement("typeList");//获得表对象
    var tr = tb.insertRow(-1);//插入一行
    tr.insertCell(-1).innerHTML = '<input type="checkbox" name="Id" value="' + model.id + '"/>';
...
function deleteRow(element) {
    ...
    $.post('/hyType_DEL',{id:delTr.childNodes[1].innerHTML}, function (data)
    {
        console.log(data);
        if(data)
        {
            typeData.deleteById(delTr.childNodes[1].innerHTML);//删除对应的数组元素
            typeData.pages = typeData.length % typeData.pageCount != 0 ? (typeData.length - typeData.length % typeData.pageCount) / typeData.pageCount + 1 : typeData.length / typeData.pageCount;
            if( typeData.pageIndex > typeData.pages)
            {
                typeData.pageIndex = typeData.pages;
            }
            loadDetailData();
        }
    });
...
var inputTypeName = document.createElement("input");//捐赠人名称
...
function setUpdateState(element) {
    ...
    txtToInput(GlobalUpdateTr.childNodes[2], inputTypeName);
...
function update(element) {
    ...
    },function ( data)
    {
        ...
        {
            //更新界面
            updateTr.childNodes[2].innerHTML = inputTypeName.value;
...
window.onload = function () {
    ...
    getElement("btnAdd").onclick = function () {
        ...
        var typeName = getElement("typeName").value;
        $.post('/hyType_ADD',{
            name:typeName
        },function ( data)
        {
    ...
    getElement("btnFirstPage").onclick = function (){
        if(typeData.pageIndex !== 1){
            typeData.pageIndex = 1;//页码++,取得下一页
            loadDetailData();
        }
    ...
    getElement("btnLastPage").onclick = function (){
        typeData.pages = typeData.length % typeData.pageCount != 0 ? (typeData.length - typeData.length % typeData.pageCount) / typeData.pageCount + 1 : typeData.length / typeData.pageCount;
        if(typeData.pages !== typeData.pageIndex)
        {
            typeData.pageIndex = typeData.pages;//页码++,取得下一页
            loadDetailData();
        }
    ...
    getElement("btnNextPage").onclick = function () {
        ...
        //获取总页数
        typeData.pages = typeData.length % typeData.pageCount != 0 ? (typeData.length - typeData.length % typeData.pageCount) / typeData.pageCount + 1 : typeData.length / typeData.pageCount;
        if (typeData.pageIndex == typeData.pages) {//判断是否最后一页
            alert("最后一页啦");
            return;//返回
        }

        typeData.pageIndex++;//页码++,取得下一页
        loadDetailData();
    };
    //5-2.上一页
    getElement("btnPrevPage").onclick = function () {
        ...
        //获取总页数
        typeData.pages = typeData.length % typeData.pageCount != 0 ? (typeData.length - typeData.length % typeData.pageCount) / typeData.pageCount + 1 : typeData.length / typeData.pageCount;
        if(typeData.pageIndex==1){//判断是否第一页
            alert("这个第一页");
            return;
        }

        typeData.pageIndex--;//页码--，取得上一页
        loadDetailData();
    };
};