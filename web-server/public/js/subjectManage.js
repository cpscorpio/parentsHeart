...
//2-4.根据捐赠单位id返回数组
subjectData.getSearchData = function (id) {
    if(id==-1){//id为-1返回所有数据
        return this;
    };
    //定义存储查询数据的数组
    ...
    for (var i = 0; i < this.length; i++) {
        if(this[i].category==id){//判断category是否符合条件
...
function loadOrgData(element) {
    for (var i = 0; i < typeData.length; i++) {
        var opt = new Option(typeData[i].typeName, typeData[i].id);
...
function addTr(model) {
    ...
    tr.insertCell(-1).innerHTML = '<input type="checkbox" name="subjectCode" value="' + model.id +'">'
    tr.insertCell(-1).innerHTML = model.id;//插入序号列
    tr.insertCell(-1).innerHTML = model.name;//插入名称列
    var td = tr.insertCell(-1);//插入category列
    td.setAttribute("category", model.category);//保存category到td的category属性（自定义）
    td.innerHTML = typeData.getTypeById(model.category).typeName;//设置捐赠单位名称
...
function deleteRow(element) {
    ...
    $.post('/fSubject_DEL',{id:delTr.childNodes[1].innerHTML}, function (data)
    {
        console.log(data);
        if(data)
        {
...
function txtToSelect(td, element) {
    ...
    element.value = td.getAttribute("category");
};
...
function rollBack(element) {
    ...
    var category = element.childNodes[3].getAttribute("category");//取得捐赠单位id
    element.childNodes[3].innerHTML = typeData.getTypeById(category).typeName;//恢复原来的文本值（捐赠单位）
...
function update(element) {
    ...
    },function ( data)
    {
        if( data)
        {
            ...
            updateTr.childNodes[3].innerHTML = typeData.getTypeById(selectType.value).typeName;
            updateTr.childNodes[3].setAttribute("category", selectType.value);//更新category
            ...
            var model = { "id": id, "name": name, "category": category};//创建与当前行数据对应的对象
...
//        var arrSearchData = subjectData.getSearchData(searchId);//得到查询数据
